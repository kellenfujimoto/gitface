...
Manage (r)ings?'''
    ...
    if nav == 'r':
        ...
        for row in \
            timeline.command('select * from stream order by timestamp desc limit 10;'
                             ):
...
timeline.printRings()
currentRing = -1
while not -1 < currentRing \
    < timeline.command('select count(*) from keys'):
